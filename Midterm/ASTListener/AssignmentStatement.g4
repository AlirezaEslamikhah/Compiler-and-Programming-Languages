...
ternerycond returns [value_attr = str(), type_attr = str()]:
        (ID | DIGIT) RELOP expr;
cond  returns [value_attr = str(), type_attr = str()]:
      expr '>' expr ;
assign returns [value_attr = str(), type_attr = str()]:
      ID ':=' expr ;
ternety returns [value_attr = str() , type_attr = str()] :
      (ID |DIGIT ) ':=' ternerycond '?' ((ID | DIGIT) ':' ternerycond '?')*  ((ID | DIGIT) ':' ternerycond);
