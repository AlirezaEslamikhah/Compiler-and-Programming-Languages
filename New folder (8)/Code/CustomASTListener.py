...
class ASTListener(AssignmentStatementListener):
    ...
    def enterAssign(self, ctx:AssignmentStatementParser.AssignContext):
        print(f'line {ctx.start.line} : {ctx.parentCtx.parentCtx.children[0].getText()}')

        # if ctx.variableDeclaratorId().dims():
        #     print(f'line {ctx.start.line}:{ctx.parentCtx.parentCtx.children[0].getText()}{ctx.variableDeclaratorId().dims().getText()} - name:{ctx.variableDeclaratorId().Identifier().getText()}')
        # else:
        #     print(f'line {ctx.start.line}:{ctx.parentCtx.parentCtx.children[0].getText()} - name:{ctx.variableDeclaratorId().Identifier().getText()}')
